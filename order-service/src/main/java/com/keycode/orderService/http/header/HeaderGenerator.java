package com.keycode.orderService.http.header;
...
public class HeaderGenerator {
    public HttpHeaders getHeadersForSuccessGetMethod() {
        ...
        httpHeaders.add("Content-Type", "applicaction/json; charset=UTF-8");
        return httpHeaders;
    }
    ...
    public HttpHeaders getHeadersForSuccessPostMethod(HttpServletRequest request, Long newResourceId) {
        HttpHeaders httpHeaders = new HttpHeaders();
        try {
            httpHeaders.setLocation(new URI(request.getRequestURL() + "/" + newResourceId));
